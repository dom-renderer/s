...
class SettingController extends Controller
{
    ...
    public function jobIndex()
    {
        ...
        $jobInspectionOrders = JobInspectionOrdering::getInspectionOrders();
            ...
            if ($request->has('job_inspection_orders')) {
                ...
                foreach ($request->job_inspection_orders as $inspectionData) {
                    if (isset($inspectionData['id']) && $inspectionData['id'] > 0) {
                        JobInspectionOrdering::where('id', $inspectionData['id'])->update([
                        ...
                        $inspectionToKeep[] = JobInspectionOrdering::create([
                            'department_id' => $inspectionData['department_id'],
                            'ordering' => $inspectionData['ordering'],
                        ])->id;
                    }
                ...
                if (!empty($inspectionToKeep)) {
                    JobInspectionOrdering::whereNotIn('id', $inspectionToKeep)->delete();
                } else {
                    JobInspectionOrdering::where('id', '>', 0)->delete();
                }
            } else {
                JobInspectionOrdering::where('id', '>', 0)->delete();
            }
